...
public class DeleteController extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            String i1="";
            DeleteBean deleteInfo = new DeleteBean( );
            deleteInfo.setitemCode(code);
            DeleteService deleteService = ServiceFactory.getDeleteService();
        try{
                i1 = deleteService.delete(deleteInfo);
           }
        ...
        if( i1.equalsIgnoreCase("Success")){
            RequestDispatcher rd = request.getRequestDispatcher("AdminHomePage.jsp");
            rd.forward(request, response);

        }

        if( i1.equalsIgnoreCase("Failure")){
            RequestDispatcher rd = request.getRequestDispatcher("AdminHomePage.jsp");
            rd.forward(request, response);
